...
def ensure_required_dependencies() -> None:
    """Exit with a helpful message before third-party imports fail."""
    missing_packages = [
        package_name
    ...
    unique_missing = ", ".join(dict.fromkeys(missing_packages))
    pip_command = f"python -m pip install {' '.join(dict.fromkeys(missing_packages))}"
    conda_command = "conda install -c conda-forge rdkit pandas numpy matplotlib scikit-learn"
    message = f"""
    [QSAR][ERROR] Missing required Python packages: {unique_missing}

    This script needs RDKit, pandas, numpy, matplotlib, and scikit-learn.
    Install the missing packages and run the script again.

    Pip example:
      {pip_command}

    Conda example (recommended for RDKit):
      {conda_command}
    """
